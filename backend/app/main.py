...
app.add_middleware(
    ...
    allow_methods=["*"], # <-- Changed to allow all methods (GET, POST, PATCH, DELETE, OPTIONS)
    allow_headers=["*"],
)
...
def analytics_resume(week: int | None = None):
    ...
    for tid, r in rows.items():
        # Only use the raw total points, no division
...
def analytics_sos(week: int | None = None):
    ...
    MIN_OPP_GAMES = 33  # Threshold to be considered for full SOS value
